...
class MainHotSale(Main):
    # 热销商品
    class Meta:
        db_table = "bees_hotsale"
...
class Goods(models.Model):
    ...
    categoryid = models.CharField(max_length=16)  # 商品的分类id
    childcid = models.CharField(max_length=16)  # 商品的子分类id
...
class CartModel(models.Model):
    ...
    iprice = models.FloatField(default=0)
...
class OrderModel(models.Model):
    ...
    # 0代表已下单，但是未付款；1代表已付款，未发货；2代表已付款，已发货
    o_status = models.ImageField(default=0)  # 状态
    o_create = models.DateTimeField(auto_now=True)  # 创建时间
    o_total = models.FloatField(default=0)
